...
################################################################################
# Simple mkdir -p with some output for tracing.
function makeDir(){
    local theDir=${1}
    if [[ -d "${theDir}" ]]; then
        printMSG "Directory at ${theDir} already exists. Doing nothing."
    else
        printMSG "Directory at ${theDir} does not exist. Creating."
        mkdir -p ${theDir} ;
    fi
}
################################################################################
# Simple rm -rf with some output for tracing and a check to make sure we are not
# removing anything outside of /opt/demo.
function rmDir(){
    local onlyInside="/opt/demo"
    local theDir=${1}
    if [[ -d "${theDir}" ]]; then
        printMSG "Directory at ${theDir} exists. Checking to make sure it is in ."
    else
        printMSG "Directory at ${theDir} does not exist. Creating."
        mkdir -p ${theDir} ;
    fi
